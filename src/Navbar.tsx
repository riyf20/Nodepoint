...
import { UserIcon, type UserIconHandle } from "./components/ui/animatedIcons/user-icon";
import { BookOpenIcon, type BookOpenIconHandle } from "./components/ui/animatedIcons/book-open-icon";
import { LogoutIcon, type LogoutIconHandle } from "./components/ui/animatedIcons/logout-icon";
...
function Navbar() {
  ...
  const hideNavBar =
    ...
    (!loggedIn && tabBarRoutesLoggedIn.includes(location.pathname));
  ...
  const tabBarOption = (index: number) => {
    return (
      ...
      >
        <span
          className={`text-transparent ${tab !== tabBarNames[index] && "group-hover:text-[#27B1FC] transition-colors duration-200"} `}
        >
          {logo[0]}{" "}
        </span>
        {tabBarNames[index]}
  ...
  const handleLogOut = async () => {
    try {
      ...
      localStorage.removeItem("trendingUsers");
    ...
    return (
      <>
        <DropdownMenu>
          ...
          <DropdownMenuContent className="relative right-5 top-6 bg-[#171718] scale-105 border border-[#27B1FC]/30 transition-all duration-250 transform hover:border-[#27B1FC]/60">
            <DropdownMenuItem
            ...
            <DropdownMenuItem
              ...
              onMouseEnter={() => {
                bookOpenIconRef.current?.startAnimation();
              }}
              onMouseLeave={() => {
                bookOpenIconRef.current?.stopAnimation();
              }}
            >
  ...
  return (
    !hideNavBar && (
      <div
        ...
        className={`relative overflow-hidden bg-[#171718]/50 backdrop-blur-md border-b transition-colors duration-100 ${isHovering ? 'border-[#27B1FC]/40' : 'border-[#27B1FC]/30'} flex flex-row items-center py-4`}
      >
        <div
        ...
        <div className="flex flex-row ml-auto gap-4 mr-6">
          {tabBarOption(0)}
          {loggedIn && tabBarOption(3)}
          {tabBarOption(1)}
          {loggedIn ? profileOption() : tabBarOption(2)}
        </div>
      </div>
    )
  );
