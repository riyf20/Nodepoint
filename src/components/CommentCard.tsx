...
import { ChevronDownIcon, type ChevronDownIconHandle } from './ui/chevron-down-icon'
...
import { deleteComment, fetchSpecificPost, getLivePicture } from '../services/appwriteServices'
import { SquareArrowOutUpRightIcon, type SquareArrowOutUpRightIconHandle } from './ui/square-arrow-out-up-right-icon'
...
function CommentCard({comment, delay, onDelete}:CommentCardProps) {
    ...
    const truncatedTitle =  comment.Body.length > 30 ? '"' + comment.Body.slice(0, 30) + '...' + '"': comment.Body
  ...
  return (
    ...
    >
        <div className='flex flex-row p-4 gap-3'>
            <div>
                {posterPicture ? 
                    <img src={miniPictureId} className='w-12 h-12 rounded-md object-cover' />
                    :
            ...
            <div className='flex-1 min-w-0'>
                ...
                <p className='font-normal text-md'>Commented on <span className='italic text-[#27B1FC] '>"{loaded && post!.Title}"</span> </p>
                <p className='text-muted-foreground'>{format(new Date(comment.$createdAt), 'MMM d')}</p>
            </div>
            <div className={`ml-auto shrink-0 flex items-center justify-center w-10 h-10 rounded-full hover:bg-[#27B1FC]/30 hover:shadow-xl transition-all duration-300 ${expanded && 'bg-[#27B1FC]/30'} `}>
                {expanded ? 
                    <ChevronUpIcon className='text-[#27B1FC]' onClick={() => {setExpanded(true)}} ref={ChevronUpIconRef} />
                        :
                    <ChevronDownIcon className='text-[#27B1FC]' onClick={() => {setExpanded(false)}} ref={ChevronDownIconRef} />
                }
            </div>
        </div>
                    ...
                    >
                        ...
                        <div className='flex-2/3'>
                            <div className='flex justify-start'>
                                <p className='font-bold text-md text-[#27B1FC]'>Full Comment</p>
                            </div>
                        ...
                        <div className='flex-1/3 flex flex-col items-center'>
                            <div className='flex justify-center'>
                                <p className='font-bold text-md text-[#27B1FC]'>Comment Actions</p>
                            </div>
                            <div className='flex flex-col gap-2 mt-4 w-1/2'>
                                ...
                                <Button
                                    ...
                                    className={`bg-[#222c36]/80 hover:bg-[#222c36]/70 transition-all duration-300 text-slate-300 hover:text-white`}
                                    onClick={() => {
        ...
        <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog} >
            <DialogContent className='bg-[#555555] border-none' onClick={(e) => {e.stopPropagation()}} >
                ...
                <DialogHeader>
                    ...
                    <DialogDescription className='text-white'>Are you sure you want to delete this comment? This action cannot be undone.</DialogDescription>
                </DialogHeader>

                <DialogFooter>
                    <Button onClick={() => {setShowDeleteDialog(false)}} disabled={updating} >Cancel</Button>
                    <Button className='text-destructive hover:bg-destructive/30' onClick={() => {handleCommentDelete()}} disabled={updating} > {updating && <Spinner/>} {updating ? 'Deleting...' : 'Confirm'}</Button>
                </DialogFooter>
